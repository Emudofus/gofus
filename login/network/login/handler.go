...
func handle_client_connection(ctx *context, client Client) {
	...
	log.Printf("[login-net-client-%04d] connected", client.Id())
...
func handle_client_disconnection(ctx *context, client Client) {
	...
	log.Printf("[login-net-client-%04d] disconnected", client.Id())
}

func handle_client_data(ctx *context, client Client, data string) {
	log.Printf("[login-net-client-%04d] RCV(%03d) %s", client.Id(), len(data), data)
	...
	default:
		...
		log.Printf("[login-net-client-%04d] unknown state", client.Id())
		client.Close()
	}
}
